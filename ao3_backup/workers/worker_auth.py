...
def run(worker_name: str = None, parallelism: int | None = None):
    ...
    def handle_one_safe(ao3_id: int) -> tuple[str, int, str]:
        ...
        """
        try:
            return handle_one_authed(ao3_id=ao3_id, cred_man=cred_man, eng=eng, name=name)
...
def handle_one_authed(ao3_id: int, cred_man: CredentialManager, eng: Engine, name: str) -> tuple[str, int, Optional[str]]:
    """
    Attempt to deal with a single id which requires auth to work - safely.
