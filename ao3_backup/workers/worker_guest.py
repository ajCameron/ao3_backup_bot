...
import concurrent.futures, os, time, json
            ...
            if outcome == "public":
                ...
                with Session(eng) as s, s.begin():
                    s.execute(
                        text(
                            """
INSERT OR REPLACE INTO works (id,status,http_status,last_seen,last_fetched,content_sha256,size_bytes,title,words,chapters,language,rating,fandoms,relationships,characters,freeform_tags,kudos,bookmarks,hits,summary,raw_meta,restricted,needs_auth)
VALUES (:id,:status,:http,datetime('now'),datetime('now'),:sha,:size,:title,:words,:chapters,:language,:rating,:fandoms,:relationships,:characters,:freeform,:kudos,:bookmarks,:hits,:summary,:raw_meta,0,0)
                    """
                        ),
                        {
                            "id": id_,
                ...
                with Session(eng) as s, s.begin():
                    ...
                    s.execute(
                        text("DELETE FROM queue WHERE id = :id AND mode='guest'"),
                        {"id": id_},
                    )
                    s.execute(
                        fetch_log.insert().values(
