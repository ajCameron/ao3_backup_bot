...
class TestWorkMetadata:
    ...
    def test_work_metadata_guest_session(self) -> None:
        ...
        assert work_metadata["characters"] == [
            "Beckett Mariner",
            "Brad Boimler",
            "Sam Rutherford",
            "D'Vana Tendi",
        ...
        assert (
            work_metadata["comments"] >= 7
        )  # No, you don't get the comments text by default
        assert work_metadata["expected_chapters"] == 5
        assert work_metadata["fandoms"] == [
        ...
        assert work_metadata["relationships"] == [
            "Beckett Mariner & T'Lyn",
            "Brad Boimler & T'Lyn",
            "Sam Rutherford & T'Lyn",
            "D'Vana Tendi & T'Lyn",
        ]
        assert work_metadata["restricted"] == False
        assert work_metadata["status"] == "Work in Progress"
        assert (
            work_metadata["summary"]
    ...
    def test_work_start_and_end_notes_subscribed_guest_session(self) -> None:
        ...
        assert (
            work.start_notes == ""
        ), "There didn't used to be start notes on this work..."
